...
def _expected_latest_trade_date(reference_date: Optional[date] = None) -> date:
    """Return the latest expected trading date using local weekend awareness.

    The project does not depend on an exchange calendar. This helper preserves
    the existing lightweight behavior while avoiding weekend refresh attempts
    when Friday's close is already stored.
    """
...
def update_ticker_prices(
    ...
    try:
        ...
        download_start = (
            _next_calendar_day(latest_date)
            if latest_date
            else (start_date or DEFAULT_PRICE_HISTORY_START_DATE)
        )
