...
def data_quality_score(inputs: GrahamInputs, extra_warnings: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    def penalty(reason: str, points: float) -> None:
    ...
    if any((item or {}).get("accepted_at_fallback_used") for item in metadata.values() if isinstance(item, dict)):
        penalty("filing_date fallback used", 10)
    if inputs.eps_method.value.startswith("ANNUAL"):
        penalty("annual EPS fallback used instead of TTM EPS", 8)
    if inputs.eps_method.value.endswith("BASIC"):
        penalty("basic EPS fallback used", 8)
    if inputs.shares_outstanding is None:
        penalty("shares outstanding unavailable", 15)
    if inputs.preferred_equity is None:
        penalty("preferred equity unavailable", 4)
    for field, points in (
        ("current_assets", 6),
        ...
        if getattr(inputs, field) is None:
            penalty(f"{field} unavailable", points)
    if "incomplete five-year earnings history" in warnings:
        penalty("incomplete five-year earnings history", 8)
    if any("future" in warning.lower() for warning in warnings):
        penalty("possible future data issue", 30)

    score = max(0.0, 100.0 - sum(item["points"] for item in penalties))
    return {
        "score": score,
...
def graham_quality_score(inputs: GrahamInputs, metrics: GrahamMetrics) -> Dict[str, Any]:
    ...
    categories = {
        "valuation": {"total": sum(valuation_rules.values()), "rules": valuation_rules},
        "financial_strength": {"total": sum(financial_rules.values()), "rules": financial_rules},
        "earnings_quality": {"total": sum(earnings_rules.values()), "rules": earnings_rules},
        "tradability": {"total": liquidity_score + cap_score, "rules": {"average_dollar_volume": liquidity_score, "market_cap": cap_score}},
        "data_quality": {"total": data_score, "rules": {"data_quality_points": data_score}},
    }
    score = float(sum(category["total"] for category in categories.values()))
