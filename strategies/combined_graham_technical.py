...
class CombinedSignalType(str, Enum):
    """Standardized signal types for the combined strategy."""
...
def _rsi(closes: Sequence[float], window: int) -> Optional[float]:
    ...
    average_gain = sum(gains) / float(window)
    average_loss = sum(losses) / float(window)
    if average_loss == 0:
        return 100.0
...
def _volatility(returns: Sequence[float]) -> Optional[float]:
...
def calculate_technical_metrics(
    ...
    config: TechnicalCapitulationConfig = TechnicalCapitulationConfig(),
) -> Tuple[TechnicalMetrics, List[str], List[str]]:
    """Calculate point-in-time technical metrics from rows on or before evaluation_date."""
    ...
    if len(rows) < minimum_rows:
        warnings.append(f"insufficient lookback: need {minimum_rows} rows, found {len(rows)}")
    ...
    one = _trailing_return(closes, 1)
    five = _trailing_return(closes, 5)
    ten = _trailing_return(closes, 10)
    twenty = _trailing_return(closes, 20)
    ...
    average_volume = sum(prior_volumes) / float(len(prior_volumes)) if prior_volumes else None
    relative_volume = current_volume / average_volume if current_volume is not None and average_volume and average_volume > 0 else None
    ...
    recent_returns = [_trailing_return(closes[:index + 1], 1) for index in range(max(1, len(closes) - 20), len(closes))]
    gap = None
    if rows:
        latest = rows[-1]
        previous_close = closes[-2] if len(closes) >= 2 else None
        latest_open = _finite(latest.get("open"))
        if previous_close and previous_close > 0 and latest_open is not None:
    ...
    metrics = TechnicalMetrics(
        ...
        moving_average,
        distance,
        _volatility([value for value in recent_returns if value is not None]),
        gap,
    )
    return metrics, warnings, source_dates


def _score_band(value: Optional[float], bands: Sequence[Tuple[float, int]], default: int = 0) -> int:
    if value is None:
        return default
    score = default
...
def calculate_panic_score(metrics: TechnicalMetrics) -> TechnicalPanicScore:
    """Return transparent panic score components."""
    components: List[PanicScoreComponent] = []
    warnings: List[str] = []
    ...
    distance_below_ma = max(0.0, -(metrics.percent_below_moving_average or 0.0)) if metrics.percent_below_moving_average is not None else None
    values = [
        ...
        ("distance_below_moving_average", distance_below_ma, _score_band(distance_below_ma, [(0.03, 1), (0.05, 2), (0.10, 3)]), "3%/5%/10%"),
    ]
    for name, observed, score, threshold in values:
        if observed is None:
        ...
        components.append(
            PanicScoreComponent(
                ...
                passed=int(score) > 0,
                explanation=f"{name} score {int(score)} from observed {observed}",
            )
        )
    return TechnicalPanicScore(sum(component.score for component in components), components, warnings)


def evaluate_technical_capitulation(
    ticker: str,
    evaluation_date: str,
    price_history: List[Dict[str, Any]],
    config: TechnicalCapitulationConfig = TechnicalCapitulationConfig(),
) -> TechnicalCapitulationEvaluation:
    """Evaluate technical capitulation using only point-in-time price rows."""
    metrics, warnings, source_dates = calculate_technical_metrics(ticker, evaluation_date, price_history, config)
    panic_score = calculate_panic_score(metrics)
    reasons: List[str] = []
    five_decline = max(0.0, -(metrics.five_day_return or 0.0)) if metrics.five_day_return is not None else None
    ten_decline = max(0.0, -(metrics.ten_day_return or 0.0)) if metrics.ten_day_return is not None else None
    distance_below_ma = max(0.0, -(metrics.percent_below_moving_average or 0.0)) if metrics.percent_below_moving_average is not None else None
    if five_decline is None or five_decline < config.minimum_five_day_decline:
        reasons.append("five_day_decline_below_minimum")
    if ten_decline is None or ten_decline < config.minimum_ten_day_decline:
    ...
    if distance_below_ma is None or distance_below_ma < config.minimum_distance_below_moving_average:
        reasons.append("moving_average_distance_below_minimum")
    if panic_score.total_score < config.minimum_panic_score:
        reasons.append("panic_score_below_minimum")
    return TechnicalCapitulationEvaluation(
        ticker.upper(),
        evaluation_date,
        metrics,
        panic_score,
        not reasons,
        sorted(set(reasons)),
        sorted(set(warnings + panic_score.warnings)),
        source_dates,
    )


def normalize_technical_score(panic_score: int) -> float:
    """Normalize the 0-15 panic score to a 0-100 scale."""
...
def evaluate_combined_candidate(
    graham_evaluation: GrahamEvaluation,
    technical_evaluation: TechnicalCapitulationEvaluation,
    config: CombinedStrategyConfig = CombinedStrategyConfig(),
    graham_signal_date: Optional[str] = None,
    technical_signal_date: Optional[str] = None,
) -> CombinedStrategyEvaluation:
    """Combine Graham and technical evaluations with transparent reasons."""
    ...
    if config.combination_mode == "weighted_composite":
        qualified = combined_score >= config.minimum_combined_score and "technical_before_graham" not in reasons and "confirmation_window_expired" not in reasons
    elif config.combination_mode == "graham_required_technical_confirms":
        qualified = not any(reason in reasons for reason in ("graham_not_qualified", "technical_not_qualified", "technical_before_graham", "confirmation_window_expired", "combined_score_below_minimum"))
    else:
        qualified = not reasons
    strong = qualified and graham_score >= 80 and technical_evaluation.panic_score.total_score >= 9
    signal_type = CombinedSignalType.STRONG_GRAHAM_TECHNICAL_CANDIDATE if strong else CombinedSignalType.GRAHAM_TECHNICAL_CANDIDATE if qualified else CombinedSignalType.NONE
    return CombinedStrategyEvaluation(
        graham_evaluation.ticker,
        technical_evaluation.evaluation_date,
        graham_evaluation,
        technical_evaluation,
        combined_score,
        config.combination_mode,
        qualified,
        "strong" if strong else "qualified" if qualified else "not_qualified",
        signal_type,
        sorted(set(reasons)),
        sorted(set(graham_evaluation.warnings + technical_evaluation.warnings)),
        f"Graham score {graham_score:.2f}, technical score {technical_score:.2f}, combined score {combined_score:.2f}",
        graham_date,
        technical_date,
    )
...
def rank_combined_candidates(evaluations: Sequence[CombinedStrategyEvaluation]) -> List[CombinedStrategyEvaluation]:
    """Rank candidates deterministically for portfolio-capacity selection."""
    return sorted(
        evaluations,
        key=lambda item: (
            -item.combined_score,
            -(item.graham_evaluation.metrics.margin_of_safety or -1.0),
            -item.graham_evaluation.metrics.graham_quality_score,
            -item.technical_evaluation.panic_score.total_score,
            -(item.graham_evaluation.inputs.average_dollar_volume_20d or 0.0),
            item.ticker,
        ),
    )
...
class CombinedGrahamTechnicalStrategy(BaseStrategy):
    """Read-only combined Graham value plus technical capitulation strategy."""
    ...
    def __init__(
        self,
        config: CombinedStrategyConfig = CombinedStrategyConfig(),
        graham_strategy: Optional[GrahamValueStrategy] = None,
        universe_config: Optional[UniverseConfig] = None,
        maximum_holding_days: int = 504,
        stop_loss_pct: Optional[float] = None,
        enable_technical_recovery_exit: bool = False,
    ) -> None:
        errors = validate_combined_strategy_config(config)
        if errors:
            raise ConfigurationValidationError(errors)
        ...
        self.graham_strategy = graham_strategy or GrahamValueStrategy(
            strategy_config=config.graham,
            universe_config=universe_config or UniverseConfig(),
            reevaluation_frequency="daily",
        )
    ...
    def evaluate(self, ticker: str, evaluation_date: str, price_history: List[Dict[str, Any]]) -> CombinedStrategyEvaluation:
        ...
        return evaluate_combined_candidate(graham, technical, self.config)
    ...
    def generate_exit_signal(self, position: Position, as_of_date: str, price_history: List[Dict[str, Any]]) -> Signal:
        ...
        if self.stop_loss_pct is not None and latest_close is not None and position.entry_price > 0:
            if percentage_return(latest_close, position.entry_price) <= -abs(self.stop_loss_pct):
        ...
        if self.enable_technical_recovery_exit:
            ...
            recovered_ma = technical.metrics.moving_average is not None and latest_price is not None and latest_price >= technical.metrics.moving_average
